...
int main(int argc, char** argv) {
	...
	try {
		...
		if (argc == 4) {
			...
			// We output the full sequence of wig3j(j, j1, j, 0, m, -m), together with timing information.
			...
			// Here we assume that the four arguments are j, j1, j2 and m.
			// We output the single value wig3j(j, j1, j, 0, m, -m).
			
			unsigned int j = atoi(argv[1]);
			unsigned int j1 = atoi(argv[2]);
			unsigned int j2 = atoi(argv[3]);
			...
			if (((j < (j12_max - j12_min))) || (j > (j12_max + j12_min))) {
				...
				std::vector<double> val_array(2 * j12_min + 1);
				uclwig3j::uclwig3j(j1, j2, m, val_array.begin());
				double wig3j = val_array[j - (j12_max - j12_min)];
				std::cout << std::scientific << std::setprecision(16) << wig3j << std::endl;
			}
		}
		else {
			throw "Must provide either three arguments (j1, j2, m) or four arguments (j, j1, j2, m)";
		}
	}
